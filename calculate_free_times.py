...
"""
most of these functions have an insane amout of arguments because I want to test these 
functions independantly from the flask app
"""

def normalize_ending_time(event, sesh_daily_end_time):
    #scales back end time if greater than daily end   
    #makes creating free time blocks easier.
    daily_end_time = sesh_daily_end_time.split(":")
...
def list_events(service, calendarIDs, sesh_id, sesh_begin_date, sesh_end_date, sesh_daily_begin_time, sesh_daily_end_time, sesh_ignoreable_events): 
    print("Entering cft.list_events")
    then = next_day(sesh_end_date)
    results = []
    for calID in calendarIDs:
        eventsResult = service.events().list(
            calendarId=calID, timeMin=sesh_begin_date, timeMax=then, singleEvents=True,
        ...
        for event in events:
            ...
            if sesh_ignoreable_events != None and event_id in sesh_ignoreable_events:
            ...
            elif str(dateTime_start.time()) <= sesh_daily_begin_time and str(dateTime_end.time()) >= sesh_daily_end_time:
                # print("Event '{}' must be nontransparent really long, almost all day event, will not skip".format(summary))
                all_day_event = True

            elif str(dateTime_end.time()) <= sesh_daily_begin_time or str(dateTime_start.time()) >= sesh_daily_end_time:
                # print("Event '{}' at {}-{} skipped, out of time range of {}-
                                                                            # summary, dateTime_start.time(), dateTime_end.time(), 
                                                                            # sesh_daily_begin_time, sesh_daily_end_time))
                continue
...
def calc_free_times(busy_blocks, sesh_id, sesh_begin_date, sesh_end_date, sesh_daily_begin_time, sesh_daily_end_time):
    ...
    daily_begin_time = sesh_daily_begin_time.split(":")
    daily_end_time = sesh_daily_end_time.split(":")
